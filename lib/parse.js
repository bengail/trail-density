...
export function itraUrlToMeta(url) {
  ...
  const itraId = parseInt(m[3], 10);
  const name = slug.replace(/\./g, " ").trim();
  const raceSlug = slug.toUpperCase().replace(/[^A-Z0-9]+/g, "_").replace(/^_+|_+$/g, "");
  const raceId = `${raceSlug}_${year}`;
  return { name, year, itraId, raceId, raceSlug };
}

export function makeRaceSlug(raceId) {
  return raceId.replace(/_\d{4}$/, "");
}

export function makeBulkRaceId(name, km, year) {
  const base = [name, km ? `${Math.round(km)}K` : null]
    .filter(Boolean).join("_").toUpperCase().replace(/[^A-Z0-9]+/g, "_").replace(/^_+|_+$/g, "");
